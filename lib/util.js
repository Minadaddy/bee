    ...
    var getList = function(options){
      ...
      options.childNodes.forEach(function(item){
        ...
        if(item.name == 'include'){
          includeList.push({type: data.type, value: data.value || data.name, matchbase: data.matchbase});
        }else if(item.name == 'exclude'){
          excludeList.push({type: data.type, value: data.value || data.name, matchbase: data.matchbase});
        }else if(item.name == 'filename'){
          includeList.push({type: 'file', value: data.value || data.name, matchbase: data.matchbase});
      ...
      return {includeList: includeList, excludeList: excludeList}
    }

    var matchItem = function (item, name) {
     
      if (!item.type || item.type == 'glob') {
        m = minimatch(name, item.value, {matchBase: true, dot: true});
